                ...
                script {
                    ...
                    container('tester') {
                        sh """
                            ...
                            if [ \$(echo "\${RANDOM} % 100" | bc) -lt \$(echo "${env.FAILURE_PROBABILITY} * 100" | bc) ]; then
                    ...
                    container('tester') {
                        sh """
                            ...
                            for i in \$(seq 1 10); do
                                ...
                                find . -name "*.txt" -exec grep "test" {} + > /dev/null
                                sleep 1
                            done
                            ...
                            # Simular error de filesystem
                            if [ \$(echo "\${RANDOM} % 100" | bc) -lt 25 ]; then
                ...
                script {
                    ...
                    container('tester') {
                        sh """
                            ...
                            # Test 1: Conectividad a servicio válido
                            ...
                            # Test 2: Intentar conectar a servicios inexistentes
                            ...
                            # Simular fallo de red crítico
                            if [ \$(echo "\${RANDOM} % 100" | bc) -lt 20 ]; then
                ...
                script {
                    ...
                    container('tester') {
                        sh """
                            ...
                            # Simular diferentes tipos de crashes
                            crash_type=\$(echo "\${RANDOM} % 4" | bc)
                            ...
                            # Decidir si crashear realmente
                            if [ \$(echo "\${RANDOM} % 100" | bc) -lt 40 ]; then
                ...
                script {
                    ...
                    container('tester') {
                        sh '''
                            ...
                            echo "   Disk IO: $(iostat 2>/dev/null | tail -1 || echo 'iostat not available')"
    ...
    post {
        always {
            script {
                ...
                // Archivar logs si están disponibles
        ...
        success {
            script {
                ...
                // slackSend(color: 'good', message: "✅ Error Testing Pipeline #${env.BUILD_NUMBER} completed successfully")
            }
        }
        ...
        failure {
            script {
                ...
                // slackSend(color: 'danger', message: "❌ Error Testing Pipeline #${env.BUILD_NUMBER} failed")
            }
        }
